...
museum = action {
  ...
  act = function()
    game._action = "museum";
    return [[Музей Сгоревших Свечей -- это здание середины девятнадцатого века, изрядно потрёпанное временем. На входе вам дают кусочек воска, на котором написан номер билета.]];
  end,
}
...
eradication_lvl1_cautious = action {
  ...
  act = function()
    ...
    take('clay_leg');
    game._action = "police";
    return [[ Осторожно контролируя вес, вы протаптываете дорожку через заросли грибов. Придётся ходить только в масках, но теперь здесь есть шансы жить.^^
...
eradication_lvl1_bold = action {
  ...
  act = function()
    ...
    take('clay_leg');
    game._action = "police";
    return [[ Разбежавшись, вы перепрыгиваете заросли и приземляетесь на пороге дома. Похоже, хозяйке придётся упражняться в прыжках, но хотя бы у неё появились шансы здесь жить.^^
    -- Спасибо! Вот вам за труды. -- Женщина протягивает изогнутый кусок пластилина. -- Говорят, за ноги Пластилиновых Творений дорого платят в музее Сгоревших Свечей. ]];
  end,
}
